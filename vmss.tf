resource "azurerm_virtual_machine_scale_set" "user28vmss" {
name = "user28vmss"
...
os_profile {
    ...
    admin_username = "user28"  ## VM 에 접속할 계정
...
ssh_keys {
    path = "/home/user28/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user01 등 
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user28-subnet1.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user28-bpepool.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool.id]
    }
        network_security_group_id = azurerm_network_security_group.user28nsg.id
}
tags = {
    environment = "staging"
    }
